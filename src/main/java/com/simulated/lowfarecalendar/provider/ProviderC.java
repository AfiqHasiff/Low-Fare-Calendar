...
public class ProviderC implements FlightProvider {
    ...
    public ProviderC(LfcProperties props) {
        this.config = props.getProviders().getProviderC();
    }
    ...
    public Optional<FareQuote> getFares(FlightQuery query) {
        ...
        if (latency > 0) {
            try {
                ...
                throw new RuntimeException("ProviderC interrupted", e);
            }
        }
        if (ThreadLocalRandom.current().nextDouble() < config.getErrorRate()) {
            throw new RuntimeException("providerC simulated failure");
        }
